#!/bin/bash
#SBATCH --job-name=smk_tsc_dec_ws_pi05
#SBATCH --output=/iris/u/mikulrai/logs/coverage_matrix_smoke/tsc_decent_workspace_pi05_%j.out
#SBATCH --error=/iris/u/mikulrai/logs/coverage_matrix_smoke/tsc_decent_workspace_pi05_%j.err
#SBATCH --time=0:30:00
#SBATCH --gres=gpu:3
...
# Coverage-matrix smoke: TSC × decent × workspace × Pi0.5 (1 seed)
# Tier 2.3 ckpts now in trash (purge 2026-05-08); known 0/60 on TABLE — smoke runs anyway.
...
# Trashed by Tier 3 purge 2026-05-08; point directly at the trash dir.
ARM0_DIR=${CKPT_BASE}/${ARM0_CFG}/pi05_d1_decent_arm0_v0.trash_1778245319_2287959/${CKPT_STEP}
ARM1_DIR=${CKPT_BASE}/${ARM1_CFG}/pi05_d1_decent_arm1_v0.trash_1778245319_2287959/${CKPT_STEP}
ARM2_DIR=${CKPT_BASE}/${ARM2_CFG}/pi05_d1_decent_arm2_v0.trash_1778245319_2287959/${CKPT_STEP}

for d in "$ARM0_DIR" "$ARM1_DIR" "$ARM2_DIR"; do
    [ -d "$d" ] || { echo "missing ckpt dir: $d"; exit 1; }
done

SERVER_LOG_DIR=/iris/u/mikulrai/logs/coverage_matrix_smoke/tsc_decent_workspace_pi05_${SLURM_JOB_ID}_servers
...
"$PREFLIGHT_PYTHON" -u ./policy/openpi_pi05/eval_decent_pi05.py \
    ...
    --wandb-tags 'eval,smoke,coverage-matrix,orion,tsc,decent,workspace,pi05'
