#!/bin/bash
# Universal DP-train launcher wrapper.
#
# Activates conda + exports the cache/wandb env vars shared by every legacy
# train launcher, then execs the Python dispatcher. SBATCH directives
# intentionally absent — `submit_train.sh` passes resources via `sbatch` CLI
# flags so they can come from the manifest.
#
# Usage:
#     ./run_train.sh <launcher_id> [--dry-run]
#     sbatch <resource_flags> ./run_train.sh <launcher_id>
set -euo pipefail
...
mkdir -p "$XDG_CACHE_HOME/jax/xla_autotune" "$TMPDIR" "$HOME/.r3m"
export XLA_FLAGS="--xla_gpu_per_fusion_autotune_cache_dir=$XDG_CACHE_HOME/jax/xla_autotune ${XLA_FLAGS:-}"
export XLA_PYTHON_CLIENT_PREALLOCATE=false
export HYDRA_FULL_ERROR=1
export WANDB_PROJECT="${WANDB_PROJECT:-diffusion-robofactory}"
...
if [ -z "${WANDB_API_KEY:-}" ]; then
    echo "[run_train.sh] WANDB_API_KEY not set and ~/.wandb_api_key missing." >&2
    exit 1
fi

cd /iris/u/mikulrai/projects/RoboFactory/robofactory
exec /iris/u/mikulrai/data/miniforge3/envs/RoboFactory/bin/python \
    -m robofactory.scripts.canonical.train.run_train "$@"
