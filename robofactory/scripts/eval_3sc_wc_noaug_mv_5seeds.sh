#!/bin/bash
#SBATCH --job-name=eval_3sc_wc_noaug_freshseed20k
#SBATCH --output=/iris/u/mikulrai/logs/eval_pi05_decent/noaug_freshseed20k_%j.out
#SBATCH --error=/iris/u/mikulrai/logs/eval_pi05_decent/noaug_freshseed20k_%j.err
...
# Fresh-seed OOD eval of Pi0.5 3SC wristcam-decent — POST-SEEDFIX, NO-AUG @ step 19999.
# Mirrors slurm_eval_pi05_freshseed_3sc_wc_decent.sh (IMPORTANT_TSC_ARM eval) EXACTLY,
# but swaps the ckpt path + config to our noaug-trained ckpts so we can do a
# head-to-head comparison on the SAME 60 OOD seeds 20000..20059.
...
echo "=== eval noaug_freshseed20k job ${SLURM_JOB_ID} on $(hostname) at $(date) ==="
...
# Configs match our training: pi05_robofactory_tsc_wc_decent_arm{0,1,2}
...
# Our new noaug ckpts (post-seedfix, image aug disabled).
ARM0_DIR=${CKPT_BASE}/${ARM0_CFG}/pi05_3sc_wc_decent_postseedfix_noaug_arm0_v1/${CKPT_STEP}
ARM1_DIR=${CKPT_BASE}/${ARM1_CFG}/pi05_3sc_wc_decent_postseedfix_noaug_arm1_v1/${CKPT_STEP}
ARM2_DIR=${CKPT_BASE}/${ARM2_CFG}/pi05_3sc_wc_decent_postseedfix_noaug_arm2_v1/${CKPT_STEP}

for d in "$ARM0_DIR" "$ARM1_DIR" "$ARM2_DIR"; do
    [ -d "$d" ] || { echo "missing ckpt dir: $d"; exit 1; }
done

SERVER_LOG_DIR=/iris/u/mikulrai/logs/eval_pi05_decent/noaug_freshseed20k_${SLURM_JOB_ID}_servers
...
OUT_DIR=/iris/u/mikulrai/logs/eval_pi05_decent/noaug_freshseed20k_${SLURM_JOB_ID}
VIDEO_DIR=${OUT_DIR}/videos
mkdir -p "$VIDEO_DIR" "$OUT_DIR"

# Same 60 fresh OOD seeds as the IMPORTANT_TSC_ARM freshseed eval, for direct comparison.
SEEDS="20000,20001,20002,20003,20004"

WANDB_NAME="Eval 3SC wristcam Pi0.5 [Decent OOD freshseed20k Postseedfix NoAug]"
...
"$PREFLIGHT_PYTHON" -u ./policy/openpi_pi05/eval_decent_pi05.py \
    ...
    --video-dir "$VIDEO_DIR" \
    --video-max 5 \
    --run-id "noaug_freshseed20k_${SLURM_JOB_ID}" \
    --wandb \
    --wandb-project openpi-robofactory \
    --wandb-name "$WANDB_NAME" \
    --wandb-tags 'eval,pi05,decent,3sc,wristcam,table-scene,freshseed,ood-audit,noaug,postseedfix'
