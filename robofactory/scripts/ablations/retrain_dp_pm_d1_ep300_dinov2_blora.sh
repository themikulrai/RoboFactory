#!/bin/bash
#SBATCH --job-name=pm_retrain_dino_blora
...
#SBATCH --output=/iris/u/mikulrai/logs/phase2_debug/pm_retrain_dino_blora_%j.out
#SBATCH --error=/iris/u/mikulrai/logs/phase2_debug/pm_retrain_dino_blora_%j.err

# Track B B.5.3a: DINOv2 ViT-B/14 frozen + LoRA on attn projections.
# Tests "fix the encoder via domain adaptation" hypothesis.
# Probe gave 36.42% feature diff (vs 0.03% random-init, 4.49% ImageNet, 6.37% R3M).
...
python ./policy/Diffusion-Policy/train.py \
  ...
  exp_name=pm-d1-ep300-dino-blora \
  logging.mode=online \
  logging.project=diffusion-robofactory \
  logging.group=dp_d1_encoder_fix \
  'logging.tags=[robot_dp,default_task,pm-d1-ep300-dino-blora,track-b,dinov2-vitb14,lora,frozen-backbone]' \
