#!/bin/bash
#SBATCH --job-name=pm_retrain_r3m
...
#SBATCH --output=/iris/u/mikulrai/logs/phase2_debug/pm_retrain_r3m_%j.out
#SBATCH --error=/iris/u/mikulrai/logs/phase2_debug/pm_retrain_r3m_%j.err

# Encoder fix: R3M-pretrained ResNet18 (manipulation-pretrained, Nair et al. 2022).
...
python ./policy/Diffusion-Policy/train.py \
  ...
  exp_name=pm-d1-ep300-r3m \
  logging.mode=online \
  logging.project=diffusion-robofactory \
  logging.group=dp_d1_encoder_fix \
  'logging.tags=[robot_dp,default_task,pm-d1-ep300-r3m,encoder-fix,r3m-pretrained]' \
