"""Tier J — proper eval for the state=qpos ckpt.

Critical bug found in eval_multi_dp.py: at update_obs (after every chunk
action), it passes `true_action` as `agent_pos`. For the state=qpos ckpt
this is wrong — the qpos ckpt was trained on `agent_pos = qpos` (finger
width in meters), but the eval feeds it the commanded action (gripper in
{-1, +1}). Same distribution-mismatch as the original bug.

This script feeds env's actual qpos (with finger width as dim 7) at every
update_obs, matching what the qpos ckpt was trained on.
"""
import sys
sys.path.append('./')
...
def _qpos_agent_pos(raw_obs, arm_id, agent_prefix):
    """Construct the 8-D agent_pos vector matching what the state=qpos converter
    wrote: [qpos[:7], qpos[7]] = [arm_joints, left_finger_width]."""
    qp = raw_obs['agent'][f'{agent_prefix}-{arm_id}']['qpos'].squeeze(0).cpu().numpy()
    # qp shape (9,) = [arm(7), finger_left, finger_right]
    return np.concatenate([qp[:7], qp[7:8]], axis=0).astype(np.float32)


def run_episode_proper(env, planner, dp_models, agent_num, seed, args,
    ...
    # Initial obs — feed qpos (with finger width) as agent_pos
        ...
        for i in range(args.max_chunk_actions):
            ...
            for id in range(agent_num):
                ...
                if action_step_dict[f'{action_prefix}-{id}'][i] == 0:
                    continue
                # CORRECT: feed qpos from env (with finger width), matching what state=qpos ckpt trained on
...
def main():
    ...
    print(f'env={env_id} agent_num={agent_num} ckpt={args.ckpt_suffix}', flush=True)
    ...
    for idx, seed in enumerate(args.seeds):
        ...
        m = run_episode_proper(env, planner, dp_models, agent_num, seed, args,
