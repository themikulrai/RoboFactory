"""Tier D eval — minimal, no RecordEpisodeMA wrapper, no wandb, no collapse-probe.

Reproduces the eval semantics of eval_multi_dp.run_episode but uses the
bare env (gym.make) without RecordEpisodeMA — the wrapper has been
crashing GPU PhysX in recent runs. Same TOPP / DP wiring as Tier C/E.
"""
import sys
sys.path.append('./')
...
def run_episode_minimal(env, planner, dp_models, agent_num, seed, args,
                         agent_prefix='panda', action_prefix='panda',
                         video_path: str = None):
    """Minimal copy of eval_multi_dp.run_episode — optional video recording via head_camera_global."""
    ...
    for id in range(agent_num):
        ...
        obs_dict = get_model_input(raw_obs, initial_qpos, id,
    ...
    while True:
        ...
        if info.get('success', False) is True:
            ...
            break
        if cnt % 10 == 0:
            print(f'  step {cnt}/{args.max_steps}  step_wall={_t.perf_counter() - _step_start:.2f}s  '
                  f'success={info.get("success", False)}', flush=True)

    # Write video if requested and frames captured.
...
def main():
    ...
    ap.add_argument('--video-dir', default=None,
                    help='If set, write head_camera_global rollout video here as seed{S}.mp4')
    args = ap.parse_args()
    ...
    print(f'env_id={env_id} agent_num={agent_num} agent_prefix={agent_prefix} action_prefix={action_prefix}',
          flush=True)
    print(f'Loaded ckpt suffix={args.ckpt_suffix}  num={args.checkpoint_num}', flush=True)
    ...
    for idx, seed in enumerate(args.seeds):
        ...
        m = run_episode_minimal(env, planner, dp_models, agent_num, seed, args,
